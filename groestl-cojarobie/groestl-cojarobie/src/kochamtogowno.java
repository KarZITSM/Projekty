...
public class kochamtogowno {
    ...
    public static String Groestl(List<byte[]> message, int blocksize){
        byte[] groestlhashed = new byte[]{};
        for (int i = 0; i < 7; i++) {
            groestlhashed=functionH(message, blocksize);
        }
    ...
    public static byte[] PermutationQ(byte[] message, int blocksize){
        ...
        int [][] matrixA=shiftLeftQ(subBytes(roundConstantsP(mapMessageToMatrixA(message,512))));
        for (int i = 0; i < matrixA.length; i++) {
            for (int j = 0; j < matrixA[i].length; j++) {
                matrixAA[i][j] = (byte) matrixA[i][j];
            }
