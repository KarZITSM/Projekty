...
public class xD {
    public static void main(String[] args) {
        System.out.println(0x07 & 0x11);
        List<byte[]> essa = divideMessage(padMessage("helagraegargraethrgraethththaeraagrgharaehrathtrrgraethththaeraagrgharaehrathtrrgraethththaeraagrgharaehrathtrththaeraagrgharaehrathtrhsthhsthsthsthstrhstrhrsynynnshnsyjsynsy",512),512);
        for (int i = 0; i < essa.size(); i++) {
            System.out.println(Arrays.toString(essa.get(i)));
        }
        System.out.println(essa.size());
        System.out.println(Groestl("wads",512));
    }
    ...
    public static String Groestl(String message, int blocksize){
        byte[] groestlhashed= new byte[64];
        for (int i = 0; i < 7; i++) {
            groestlhashed=functionH(divideMessage(padMessage(message,blocksize),blocksize), blocksize);
        }
        return byteArrayToHexString(functionOmega(groestlhashed));
    }
    ...
    public static byte[] functionH(List<byte[]> message, int blocksize){
        byte[] iv = new byte[64];
        byte[] output = new byte[64];
        for (int i = 0; i < message.size(); i++) {
            if(init==0){ iv = ByteBuffer.allocate(4).putInt(initialVector).array();
            init++;
            }
            //else iv = functionF(message.get(i-1),iv,blocksize);
            output =functionF(message.get(i),output,blocksize);
        }
        return output;
    }

    public static byte[] functionOmega (byte[] message){
        byte[] outputofP = PermutationP(message,512);
    ...
    public static byte[] functionF (byte[] message,byte[] iv, int blocksize){
        ...
        PermutationQ(message, blocksize);
        PermutationP(hi, blocksize);

        for (int i = 0; i < message.length; i++) {
            hi[i] = (byte) (message[i] ^ iv[i] ^ hi[i]);
        }
    ...
    public static byte[] PermutationP(byte[] message, int blocksize){
        ...
        byte[][] matrixAA = new byte[8][8];


        int [][] matrixA=shiftLeftP(subBytes(roundConstantsP(mapMessageToMatrixA(message,512))));
        ...
        byte[] output =mapMatrixToByteArray(matrixAA,blocksize);
    ...
    public static byte[] PermutationQ(byte[] message, int blocksize){
        ...
        int [][] matrixA=shiftLeftQ(subBytes(roundConstantsQ(mapMessageToMatrixA(message,512))));
        for (int i = 0; i < matrixA.length; i++) {
            for (int j = 0; j < matrixA[i].length; j++) {
                matrixAA[i][j] = (byte) matrixA[i][j];
            }
        ...
        return mapMatrixToByteArray(matrixAA,blocksize);
    }
//    public static List<byte[]> divideMessage(byte[] message,int blockSize) {
//        List<byte[]> dividedMessage = new ArrayList<>();
//
//        int totalBlocks = (message.length + blockSize - 1) / blockSize;
//
//        for (int i = 0; i < totalBlocks; i++) {
//            int startIndex = i * blockSize;
//            int endIndex = Math.min(startIndex + blockSize, message.length);
//
//            byte[] block = new byte[endIndex - startIndex];
//            System.arraycopy(message, startIndex, block, 0, block.length);
//
//            dividedMessage.add(block);
//        }
//
//        return dividedMessage;
//    }
    public static List<byte[]> divideMessage(byte[] byteArray, int subarraySize) {
        List<byte[]> dividedArray = new ArrayList<>();

        int totalSubarrays = (byteArray.length + subarraySize - 1) / subarraySize;

        for (int i = 0; i < totalSubarrays; i++) {
            int startIndex = i * subarraySize;
            int endIndex = Math.min(startIndex + subarraySize, byteArray.length);

            byte[] subarray = new byte[endIndex - startIndex];
            System.arraycopy(byteArray, startIndex, subarray, 0, subarray.length);
