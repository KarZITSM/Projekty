...
//    }
    ...
    public static void shiftLeftP(int[][] matrix){
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < i; j++) {
    ...
    public static void shiftLeftQ(int[][] matrix){
    ...
    public byte[][] mixBytes( byte[][] msg, int columns )  //mixBytes zaczerpnięte z języka C i przepisane na Jave
    {
    ...
    public static byte[] PermutationP(String message, int blocksize){
        ...
        for (int i = 0; i < messageinput.length; i++) {
            essa[i] = (byte) (messageinput[i] ^ iv[i]);
        }
        int[][] matrixA =mapMessageToMatrixA(essa,512);
