...
class Controller:
    def __init__(self):
        ...
        self.filexec = pd.read_csv("csv/session.csv", sep=";") #plik z listą portów na bazie komendy links w mininecie
        ...
        self.ip2 = ""
        self.steps = []
        self.colour = ['red','blue','green','yellow']
        self.colour_nr = 0
...
# 6 tcp lub 17 udp
        ...
        for y in range(len(self.steps[self.enumerate])-1):
            for x in range(self.ports.shape[0]):
                if (self.ports.at[x, "host1"] == self.steps[self.enumerate][z] and self.ports.at[x, "host2"] == self.steps[self.enumerate][z + 1]):
                    data_scheme_copy = copy.deepcopy(data_scheme)
                    data_scheme_copy["treatment"]["instructions"][0]["port"] = str(self.ports.at[x, "port"])
                    data_scheme_copy["selector"]["criteria"][1]["ip"] = str(self.ip2)
                    data_scheme_copy["selector"]["criteria"][2]["ip"] = str(self.ip1)
                ...
                if (self.ports.at[x, "host2"] == self.steps[self.enumerate][z] and self.ports.at[x, "host1"] == self.steps[self.enumerate][z+1]):
                    data_scheme_copy2 = copy.deepcopy(data_scheme)
                    data_scheme_copy2["treatment"]["instructions"][0]["port"] = f"{self.ports.at[x, 'port']}"
                    data_scheme_copy2["selector"]["criteria"][1]["ip"] = str(self.ip1)
                    data_scheme_copy2["selector"]["criteria"][2]["ip"] = str(self.ip2)
                    data_scheme_copy2["deviceId"] = f"of:000000000000000{hex(int(self.steps[self.enumerate][z+1][1:]))[2:]}"

                    self.data['flows'].append(data_scheme_copy2)
            z += 1
    ...
    def sendJSON(self):
        payload = open(f"json/{self.steps[self.enumerate][0]}{self.steps[self.enumerate][-1]}.json", "rb")
        print(self.data)
        response = requests.post("http://192.168.0.104:8181/onos/v1/flows", data=payload, headers={ "Content-Type": "application/json", "Accept": "application/json"}, auth=("onos", "rocks"))
    ...
    def paintGraphWithStream(self):
        ...
        if(self.colour_nr==len(self.sessions)):
